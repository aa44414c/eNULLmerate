#!/bin/bash
# Bash script to run specific commands in Nmap for
...
/usr/bin/nmap -iL $1amass_ipaddr_list.txt -T3 -Pn -oN $2nmap_amass_output.txt
echo -e "${ORANGE}-${NC} Executing ${WHITE}/usr/bin/nmap -iL $1assetfinder_ipaddr_list.txt -T3 -oN $2nmap_assetfinder_output.txt${NC}..."
/usr/bin/nmap -iL $1assetfinder_ipaddr_list.txt -T3 -Pn -oN $2nmap_assetfinder_output.txt
echo -e "${ORANGE}-${NC} Executing ${WHITE}/usr/bin/nmap -iL $1nikto_ipaddr_list.txt -T3 -oN $2nmap_nikto_output.txt${NC}..."
/usr/bin/nmap -iL $1nikto_ipaddr_list.txt -T3 -Pn -oN $2nmap_nikto_output.txt
